        ...
        foreach(DIR ${OVRTX_RUNTIME_DIRS})
            add_custom_command(TARGET ${TARGET_NAME} POST_BUILD
                COMMAND ${CMAKE_COMMAND} -E env powershell -NoProfile -Command
                    "if (-not (Test-Path '$<TARGET_FILE_DIR:${TARGET_NAME}>/${DIR}')) { New-Item -ItemType Junction -Path '$<TARGET_FILE_DIR:${TARGET_NAME}>/${DIR}' -Target '${ovrtx_SOURCE_DIR}/bin/${DIR}' | Out-Null }"
            )
        endforeach()
    else()
        # Set rpath so the executable can find libovrtx-dynamic.so at runtime
        set_target_properties(${TARGET_NAME} PROPERTIES
            BUILD_RPATH "${ovrtx_SOURCE_DIR}/bin"
            INSTALL_RPATH "${ovrtx_SOURCE_DIR}/bin"
        )
    endif()
endfunction()

...
#ifdef RADAR_EXAMPLE_USDA_PATH
constexpr char kDefaultSceneFilePath[] = RADAR_EXAMPLE_USDA_PATH;
#else
constexpr char kDefaultSceneFilePath[] = "radar_example.usda";
#endif
...
// Move the USD stage to a specific animation time and wait until the update is
// visible to the next render step.
bool update_stage_time_and_wait(ovrtx_renderer_t* renderer, double usd_time)
{
    // Stage time controls the animated cube position in radar_example.usda.
    ovrtx_enqueue_result_t enqueue_result =
        ovrtx_update_stage_from_usd_time(renderer, usd_time);
    if (!check_api_result(enqueue_result, "update_stage_from_usd_time")) {
        return false;
    }
    return wait_for_success(renderer,
                            enqueue_result.op_index,
                            "update_stage_from_usd_time");
}
// [/snippet:update-stage-time-helper]
...
int finish(ovrtx_renderer_t* renderer, int exit_code)
{
    if (renderer) {
        ...
        if (!check_api_result(result, "destroy_renderer")) {
            // Cleanup failures should make the process fail even when rendering
            // itself succeeded.
            return 1;
        }
    }
...
int main(int argc, char* argv[])
{
    ...
    std::cout << "Loading radar scene from " << scene_file_path << "...\n";
    enqueue_result =
    ...
    // Start at time 0 and render a few warmup frames without reading outputs.
    if (!update_stage_time_and_wait(renderer, 0.0)) {
        return finish(renderer, 1);
    }
    ...
    for (int step = 1; step <= kMotionStepCount; ++step) {
        ...
        if (!update_stage_time_and_wait(
                renderer, static_cast<double>(step) * kStepDeltaTimeSeconds)) {
            return finish(renderer, 1);
        }

        ovrtx_step_result_handle_t step_handle = OVRTX_INVALID_HANDLE;

        // Render one radar frame and keep the result handle for fetching.
        enqueue_result =
            ovrtx_step(renderer,
                       render_products,
                       kStepDeltaTimeSeconds,
